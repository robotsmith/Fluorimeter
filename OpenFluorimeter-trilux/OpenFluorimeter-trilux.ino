/*
   ...
   Note HFSM: Ajout de la version b avec le fluorimetre sur un module d'extension I2C vers UART
              https://wiki.dfrobot.com/Gravity%3A%20IIC%20to%20Dual%20UART%20Module%20SKU%3A%20DFR0627
              ETAT: fonctionnel, a tester avant prod
   Reste a faire :
...
char versoft[] = "1.6b";             // version du code

#define TIME_TO_SLEEP  1           // Durée d'endormissement entre 2 cycles complets de mesures (in seconds) par défault 900 = 15min
...
#include <DFRobot_IICSerial.h>          // pour interface I2C UART DFROBOT
...
const unsigned int BATTERY_CAPACITY = 6600; // e.g. 3400mAh battery
unsigned int soc ;
unsigned int volts;
...
void setup() {
  ...
  Serial.println("I2C to UART initialized.");
  ...
  if (bootCount == 0) //Run this only the first time
  {
    ...
    // affichageintro(); // TEMPORAIRE: Afin d'utiliser sans affichage
    delay (2000);
    ...
    //message1();  // TEMPORAIRE: Afin d'utiliser sans affichage
...
void mes_fluo() {
  ...
  // TODO: Ajouter un timeout afin d'éviter les blocages
